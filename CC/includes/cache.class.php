...
class CACHE {
	...
	public function read() {
		...
		global $DB;
		$row=$DB->get_row("SELECT v FROM lan_config WHERE k='cache' limit 1");
		return $row['v'];
	}
	public function save($value) {
		...
		return $DB->query("update lan_config set v='$value' where k='cache'");
	}
	...
	public function update() {
		global $DB;
		$cache = array();
		$query = $DB->query('SELECT * FROM lan_config where 1');
	...
	public function clear() {
		global $DB;
		return $DB->query("update lan_config set v='' where k='cache'");
	}
}
